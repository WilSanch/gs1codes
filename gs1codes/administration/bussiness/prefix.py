...
from administration.bussiness.models import PrefixId, ActivationInactivationBM, MarkCodeRespose, CodeAssignmentRequest, CodeAssignation, RegroupBM, CodeTransfer, TransferProcess
...
def total_transfer(enterpriseO: Enterprise, enterpriseD: Enterprise, observation, user):
    obj_prefix: Prefix = Prefix.objects.filter(enterprise_id= enterpriseO.id,state_id=StCodes.Asignado.value)

    for prefix in obj_prefix:
        prefix.enterprise_id = enterpriseD.id
        prefix.observation = "FUSIÓN: " + observation
...
def code_transfer(data: CodeTransfer):
    try:
